...
$deposit_so_far=DepositSoFar($stand);
...
//echo $mwedzi; exit;
		//payment of deposit credit entry
		 ...
		 mysql_query("insert into payment(date,cash,stand,capturer,month,year,payment_type,payment_date,value_date,d) values(NOW(),'$_POST[amount]','$stand','$_SESSION[name]','$mwedzi','$year','Deposit','$_POST[date]','$_POST[date]','Stand Deposit')") or die(mysql_error());
		 //instalments
       if($total!=$deposit){
		/* mysql_query("update stand set status='Deposit_Outstanding',datestatus='$_POST[date]' where id_stand='$stand'") or die (mysql_error());
   */
		   }else{
		 //changin status
        $purchsedate=GMfirstpayment($stand);
 mysql_query("update stand set status='Payment_In_Progress',datestatus='$purchsedate' where id_stand='$stand'") or die (mysql_error());
		   }
